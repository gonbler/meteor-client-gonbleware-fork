...
public class FastRebreak extends Module {
    ...
    public FastRebreak() {
        super(Categories.Player, "fast-rebreak", "Allows you to quickly mine multiple blocks?");
    }
    ...
    private void onTick(TickEvent.Pre event) {
            ...
            if ((mc.world.getBlockState(x.blockPos).isAir())
                ...
                mc.getNetworkHandler()
                        .sendPacket(new PlayerActionC2SPacket(
                                PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK, x.blockPos,
                                x.breakDreiction));

                return true;
            }

            if (x.timesTriedBreak > 40 && !x.beenAir) {
                return true;
            }
        ...
        for (FastRebreakBlock block : blockQueue) {
            ...
            double breakingSpeed =
                    BlockUtils.getBlockBreakingSpeed(slot.found() ? slot.slot() : 0, blockState);
            ...
            if (block.isReady()) {
                ...
                mc.getNetworkHandler()
                        .sendPacket(new PlayerActionC2SPacket(
                                PlayerActionC2SPacket.Action.STOP_DESTROY_BLOCK, block.blockPos,
                                block.breakDreiction));

                block.timesTriedBreak++;
            }
        }
    ...
    public void onStartBreakingBlock(StartBreakingBlockEvent event) {
        ...
        if (blockQueue.size() > 1) {
            FastRebreakBlock first = blockQueue.getFirst();
            mc.getNetworkHandler()
                    .sendPacket(new PlayerActionC2SPacket(
                            PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK, first.blockPos,
                            first.breakDreiction));
    ...
    class FastRebreakBlock {
        ...
        public void startBreaking() {
            ...
            double breakingSpeed = BlockUtils.getBlockBreakingSpeed(slot.found() ? slot.slot() : 0,
